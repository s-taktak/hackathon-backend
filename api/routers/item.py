    ...
    if files:
        try:
            ...
            async def upload_file(file: UploadFile):
                ...
                await run_in_threadpool(blob.upload_from_file, file.file, content_type=file.content_type)
                return f"https://storage.googleapis.com/{BUCKET_NAME}/{filename}"

            image_urls = await asyncio.gather(*(upload_file(f) for f in files))
